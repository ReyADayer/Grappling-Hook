...
import com.snowgears.grapplinghook.listener.GrappleListener;
...
public final class GrapplingHook extends JavaPlugin {
    ...
    public final GrappleListener alisten = new GrappleListener(this);
    ...
    public void onEnable(){
        ...
        if(!disableCrafting){
            ShapedRecipe woodRecipe = new ShapedRecipe(GrappleAPI.createGrapple(woodUses))
                    .shape(" **", " &*", "   ")
                    .setIngredient('*', Material.WOOD, -1)
                    .setIngredient('&', Material.FISHING_ROD);

            ShapedRecipe stoneRecipe = new ShapedRecipe(GrappleAPI.createGrapple(stoneUses))
                    .shape(" **", " &*", "   ")
                    .setIngredient('*', Material.COBBLESTONE)
                    .setIngredient('&', Material.FISHING_ROD);

            ShapedRecipe ironRecipe = new ShapedRecipe(GrappleAPI.createGrapple(ironUses))
                    .shape(" **", " &*", "   ")
                    .setIngredient('*', Material.IRON_INGOT)
                    .setIngredient('&', Material.FISHING_ROD);

            ShapedRecipe goldRecipe = new ShapedRecipe(GrappleAPI.createGrapple(goldUses))
                    .shape(" **", " &*", "   ")
                    .setIngredient('*', Material.GOLD_INGOT)
                    .setIngredient('&', Material.FISHING_ROD);

            ShapedRecipe diamondRecipe = new ShapedRecipe(GrappleAPI.createGrapple(diamondUses))
                    .shape(" **", " &*", "   ")
        ...
        if(args.length == 1){
            if(sender instanceof Player){
                ...
                if ((cmd.getName().equalsIgnoreCase("gh") && args[0].equalsIgnoreCase("give"))) {
                    if(player.hasPermission("grapplinghook.command.give"))
                        player.setItemInHand(GrappleAPI.createGrapple(50));
                    else
            ...
            if(sender instanceof Player){
                ...
                if ((cmd.getName().equalsIgnoreCase("gh") && args[0].equalsIgnoreCase("give") && args[1].length() > 0)) {
                    if(player.hasPermission("grapplinghook.command.give")){
                        if(isInteger(args[1]))
                            player.setItemInHand(GrappleAPI.createGrapple(Integer.parseInt(args[1])));
                        else if(Bukkit.getPlayer(args[1]) != null){
                            Bukkit.getPlayer(args[1]).getInventory().addItem(GrappleAPI.createGrapple(50));
                    ...
                    if(player.hasPermission("grapplinghook.command.give")){
                        if(isInteger(args[2])){
                            if(Bukkit.getPlayer(args[1]) != null){
                                ...
                                Bukkit.getPlayer(args[1]).getInventory().addItem(GrappleAPI.createGrapple(uses));
